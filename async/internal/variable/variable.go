...
type variable[T any] struct {
	...
	promise opt.Opt[promise.Promise[T]]
}

func newVariable[T any]() *variable[T] {
	return &variable[T]{
		promise: opt.New(promise.New[T]()),
	}
}
...
// GetWait returns the current value or waits for it or an error.
func (v *variable[T]) GetWait(ctx context.Context) (T, status.Status) {
...
func (v *variable[T]) get() (T, bool, status.Status, opt.Opt[promise.Promise[T]]) {
	v.mu.RLock()
	defer v.mu.RUnlock()
