...
type orderedMap[K comparable, V any] struct {
	list []OrderedMapItem[K, V] // array list
...
func newOrderedMap[K comparable, V any](items ...OrderedMapItem[K, V]) *orderedMap[K, V] {
	m := &orderedMap[K, V]{
		list: make([]OrderedMapItem[K, V], 0, len(items)),
...
func newMapSize[K comparable, V any](size int) *orderedMap[K, V] {
	return &orderedMap[K, V]{
		list: make([]OrderedMapItem[K, V], 0, size),
...
func (m *orderedMap[K, V]) Items() []OrderedMapItem[K, V] {
	items := make([]OrderedMapItem[K, V], len(m.list))
	copy(items, m.list)
	return items
}
